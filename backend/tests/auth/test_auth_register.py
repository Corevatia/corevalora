...
def test_register_duplicate_email(client, user):
    resp = client.post(
        "/auth/register", json={"email": user.email, "password": "testtest"}
    )
...
def test_register_password_too_long(client):
    resp = client.post(
        "/auth/register", json={"email": "test@test.com", "password": "x" * 73}
    )
...
def test_register_password_too_short(client):
    resp = client.post(
        "/auth/register", json={"email": "test@test.com", "password": "x" * 7}
    )
...
def test_register_invalid_email(client):
    resp = client.post(
        "/auth/register", json={"email": "testtest.com", "password": "x" * 8}
    )
