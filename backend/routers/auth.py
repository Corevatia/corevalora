...
from models.auth import LoginIn, RegisterIn, UserOut
from services.auth.passwords import hash_password, verify_dummy, verify_password
...
@limiter.limit("5/minute", key_func=get_remote_address)
def login(
    request: Request, data: LoginIn, response: Response, db: Session = Depends(get_db)
):
